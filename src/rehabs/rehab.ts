...
export const rehabs: rehabCenter[] = [
    {
  ...
  export function distance(lat: number, long: number, rehab_center: rehabCenter) {
      return Math.sqrt(Math.pow(lat-rehab_center.lat, 2) + Math.pow(long-rehab_center.long, 2))
  }
  
  export function findClosest(lat: number, long: number) {
      let closest = 999;
      let center = -1;
